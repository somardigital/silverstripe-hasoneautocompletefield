<div id="$HolderID" class="form-group field<% if $extraClass %> $extraClass<% end_if %>"
     data-default-text="$defaultText" data-autocomplete-delay="$getAutocompleteDelay">
    <% if $Title %>
    ...
    <div class="form__field-holder<% if not $Title %> form__field-holder--no-label<% end_if %>">
        ...
        <% if $Description %>
            <p class="form__field-description form-text" id="describes-$ID">$Description</p><% end_if %>
    </div>
